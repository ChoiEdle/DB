...
select truncate(rand()*100,0) from dual;
-- 정수 3자리 난수 발생(0 ~ 999) 난수 발생
...
-- 사번, 사원명, 사원명2 칼럼을 생성하여 조회
-- 사원명2 칼럼의 형식은 s0001(홍길동) 출력
...
select substring("대한민국 홍길동", 1, 5), substring("대한민국 홍길동", 6, 3) from dual;

-- 사원테이블의 사번, 사원명, 입사년도, 입사월, 입사일, 급여를 조회
select emp_id, emp_name, substring(hire_date, 1, 4) as '입사년', substring(hire_date, 6, 2) as '입사월', substring(hire_date, 9, 2) as '입사일', salary 
from employee;
...
from employee where left(hire_date, 4) between '2015' and '2017' order by emp_id desc;
...
-- convert(변환하고자하는 값, 데이터 타입) : MySQL에서 지원하는 OLD
...
select now() as date, cast(now() as char) as string, cast(cast(now() as char) as date) as date,
...
select * from employee where hire_date >= convert('20150101', date);

-- '20150101' ~ '20171231' 사이에 입사한 사원들을 모두 조회
-- 모든 mysql 데이터베이스에서 적용 가능한 형태로 작성
select * from employee where hire_date between convert('20150101', date) and convert('20171231', date);

select convert(convert(curdate(), char), datetime) as date from dual;
...
-- 소수점은 절삭
select concat('$', format(avg(salary),0)) as 평균 from employee;
...
-- '2015'년도에 입사한 입사자 수
select count(hire_date) from employee where left(hire_date,4) = '2015';

-- 정보시스템(sys) 부서의 사원수
select count(*) from employee where dept_id = 'sys';

-- 가장 빠른 입사자, 가장 늦은 입사자를 조회
...
having sum(salary) >= 30000
order by left(hire_date,4) desc;
...
select 	if(grouping(dept_id), '총합계', ifnull(dept_id, '-')) as 부서명, 
		count(*) as 사원수, 
		sum(ifnull(salary,0)) as 총급여, 
		avg(ifnull(salary,0)) as 평균급여 
from employee
...
select e.emp_id, e.emp_name, d.dept_id, d.dept_name, e.hire_date, e.salary
...
where e.dept_id = d.dept_id and e.emp_id = v.emp_id and d.dept_name = '인사';

-- 영업부서 사원의 사원명, 폰번호, 부서명, 휴가사용 이유 조회
select e.emp_id, e.emp_name, e.phone, d.dept_name, v.reason, u.unit_name
from employee e, vacation v, department d, unit u
where u.unit_id = d.unit_id and d.dept_id = e.dept_id and e.emp_id = v.emp_id
and d.dept_name = '영업';
...
-- 2014년도부터 2016년도까지 입사한 사원들 중에서 퇴사하지 않은 사원들의
...
-- 본부별, 부서의 휴가사용 일수
select u.unit_name, d.dept_name, d.dept_id, sum(e.salary), avg(e.salary), sum(v.duration)
from employee e, department d, vacation v, unit u
where e.dept_id = d.dept_id and e.emp_id = v.emp_id and d.unit_id = u.unit_id
group by d.dept_name, d.dept_id, u.unit_name;

-- outer join : inner join + 조인에서 제외된 row(테이블별 지정)
-- 오라클 형식의 outer join은 사용불가, ansi sql 형식 사용가능!!
-- select [컬럼리스트] from [테이블명1 테이블별칭] left/right outer join 테이블명2 [테이블별칭], ... on[테이블명1.조인컬럼 = 테이블명2.조인컬럼]
-- ** 오라클 형식 outer join mysql에선 사용 불가
-- select * from table1 t1, table2 t2
-- where t1.col(+) = t2.col;  // where t1.col = t2.col(+);
...
select distinct  u.unit_name, d.dept_name, count(v.duration)
...
-- char(고정형 문자형) : 크기가 메모리에 고정되는 형식, 예) char(10) --> 3자리만 입력 : 7자리 낭비
-- varchar(가변형 문자형) : 실제 저장되는 데이터 크기에 따라 메모리가 변경되는 형식 
-- 						varchar(10) --> 3자리 입력 : 메모리에 실제 3자리 공간만 생성
...
select sj.sname as 과목명, st.sname as 학생명
from student st, subject sj
where st.sid = sj.sid
and st.sname = '홍길동';
...
select s.sname
from professor p inner join subject s
on p.SID = s.SID
and p.name = '스미스';
...
-- 과목아이디, 과목명, 교수명, 학생명
select sj.sid, sj.sname, p.name, st.sname
from subject sj, professor p, student st
where sj.sid = p.sid and sj.sid = st.sid
and sj.sname = 'java';

select sj.sid, sj.sname, p.name, st.sname
from subject sj inner join professor p
on sj.sid = p.sid 
...
create table `order`(
    ...
    constraint	fk_member_id_order	foreign key(member_id)
...
select p.name, count(*) as count
from product p inner join orderitem i
...
select *
from product p, `order` o, orderitem i
where i.product_id = p.product_id and i.order_id = o.order_id;
...
-- 상품별 주문 건수, 모든 상품 조회
-- 상품명, 주문건수 출력
select p.name, count(i.quantity) as count
...
select m.name, m.created_at, o.order_date, i.quantity, p.name, p.price
from member m, product p, `order` o, orderitem i
where m.member_id = o.member_id and i.order_id = o.order_id and i.product_id = p.product_id;
...
from (select distinct m.name, m.created_at, o.order_date, i.quantity, i.product_id
		...
		where m.member_id = o.member_id 
        and i.order_id = o.order_id) t1 right outer join product p
...
emp_id, emp_name, concat(left(hire_date,4), '년도') as hire_date, phone, email, concat(format(salary,0), '만원') as salary
from employee;
...
-- 석차를 구하는 함수
select row_number() over(order by emp_id desc) as no, rank() over(order by salary desc) as r, emp_id, emp_name, dept_id, salary
from employee;
...
select ifnull(max(convert(right(mid,4), unsigned)),0)
into max_code
from trg_member;
...
-- 최재의 실험실
select cast(concat('S0',right(mid,4)) as unsigned) from trg_member;
select concat('S0',lpad(right(mid,4), 5, '0')) from trg_member;
select cast(concat('S0',lpad(right(mid,4), 5, '0')) as unsigned) from trg_member;
--
...
-- 현재 저장된 값 중 가장 큰 값을 가져옴
select ifnull(max(convert(right(emp_id,4), unsigned)),0)
into max_code
from employee_stru;
...
values('고길동', 'M', curdate(), 'SYS', '010-4564-5645', 'go@gil.dong', 1000);
select * from employee_stru;
...
create table reply(
    ...
    constraint fk_reply_bid	foreign key(bid)
				references board(bid) on delete cascade on update cascade
);
desc reply;
select * from board;
...
values("고길동", "go@gil.dong", now());

update member
set name = '홍길순', email = 't@es.t'
where member_id = 1;
...
create table score_member(
	mid			char(5)		primary key,	-- 'M0001' 형식의 트리거 생성
    name		varchar(10)	not null,
...
select * from information_schema.triggers;
delete from score_member where mid = 'M0003';
