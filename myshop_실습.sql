...
select customer_name, customer_id, gender, city, phone, point from customer where birth_date between '1990-01-01' and '1990-12-31';
-- Q21) 1990년에 출생한 여자 고객의 이름, 아이디, 성별, 지역, 전화번호, 생일, 포인트를 조회하세요.
select customer_name, customer_id, gender, city, phone, point from customer where gender = 'f' and birth_date between '1990-01-01' and '1990-12-31';
-- Q22) 1990년에 출생한 '대구' 또는 '경주' 지역 남자 고객의 이름, 아이디, 성별, 지역, 전화번호, 생일, 포인트를 조회하세요.
select customer_name, customer_id, gender, city, phone, point from customer where gender = 'm' and (city = '대구' or city = '경주') and birth_date between '1990-01-01' and '1990-12-31';
...
select customer_name, customer_id, gender, city, phone, point from customer where customer_name like '% %';
-- Q13) 전화번호가 010으로 시작하지 않는 고객의 이름, 아이디, 성별, 지역, 전화번호, 포인트를 조회하세요.
select customer_name, customer_id, gender, city, phone, point from customer where left(phone,3) not like '010';
-- Q14) 포인트가 500,000 이상 '서울' 지역 고객의 이름, 아이디, 성별, 지역, 전화번호, 포인트를 조회하세요.
select customer_name, customer_id, gender, city, phone, point from customer where point >= 500000 and city = '서울';
-- Q15) 포인트가 500,000 이상인 '서울' 이외 지역 고객의 이름, 아이디, 성별, 지역, 전화번호, 포인트를 조회하세요.
...
-- Q23) 1990년에 출생한 남자 고객의 이름, 아이디, 성별, 지역, 전화번호, 생일, 포인트를 조회하세요.
--      단, 홍길동(gildong) 형태로 이름과 아이디를 묶어서 조회하세요.
select concat(customer_name,'(', customer_id,')') as name, gender, city, phone, birth_date, point from customer 
...
select left(order_date,4) as 주문연도, sum(total_due) from order_header group by left(order_date,4);

-- Q18) 2019.01 ~ 2019.06 기간 주문에 대하여 주문연도별, 주문월별 전체금액 합을 조회하세요.
select left(order_date,4) as 주문연도, substring(order_date,6,2) as 주문월, sum(total_due) 
from order_header
...
select left(order_date,4) as 주문연도, substring(order_date,6,2) as 주문월, sum(total_due) , avg(total_due)
from order_header
where left(order_date,7) between '2019-01' and '2019-06'
group by left(order_date,4), substring(order_date,6,2);

-- Q20) 주문연도별, 주문월별 전체금액 합과 평균을 조회하고, rollup 함수를 이용하여 소계와 총계를 출력해주세요.
select 	left(order_date,4) as 주문연도, 
		substring(order_date,6,2) as 주문월, 
...
select o.order_id, o.customer_id, o.employee_id, e.employee_name, o.order_date, o.total_due
from order_header o inner join employee e
...
select o.order_id, c.city, o.customer_id, c.customer_name, o.employee_id, e.employee_name, o.order_date, o.total_due
from order_header o, employee e, customer c
where o.employee_id = e.employee_id and o.customer_id = c.customer_id and total_due >= 8500000 and c.city = '서울';
select o.order_id, c.city, o.customer_id, c.customer_name, o.employee_id, e.employee_name, o.order_date, o.total_due
from order_header o inner join employee e
on o.employee_id = e.employee_id 
inner join customer c
on o.customer_id = c.customer_id 
where total_due >= 8500000 and c.city = '서울';

-- Q06) 위에서 작성한 쿼리문을 복사해 붙여 넣은 후 전체금액이 8,500,000 이상인 '서울' 지역 '남자' 고객만 조회되게 수정하세요.
select o.order_id, c.city, o.customer_id, c.customer_name, c.gender, o.employee_id, e.employee_name, o.order_date, o.total_due
from order_header o, employee e, customer c
where o.employee_id = e.employee_id and o.customer_id = c.customer_id 
and total_due >= 8500000 and c.city = '서울' and c.gender = 'm';
select o.order_id, c.city, o.customer_id, c.customer_name, c.gender, o.employee_id, e.employee_name, o.order_date, o.total_due
from order_header o inner join employee e
on o.employee_id = e.employee_id 
inner join customer c
on o.customer_id = c.customer_id 
where total_due >= 8500000 and c.city = '서울' and c.gender = 'm';

-- Q07) 주문수량이 30개 이상인 주문의 주문번호, 상품코드, 주문수량, 단가, 지불금액을 조회하세요.
select order_id, product_id, order_qty, unit_price, line_total 
from order_detail where order_qty >= 30;

-- Q08) 위에서 작성한 쿼리문을 복사해서 붙여 넣은 후 상품이름도 같이 조회되도록 수정하세요.
select o.order_id, o.product_id, p.product_name, o.order_qty, o.unit_price, o.line_total 
...
select e.employee_id, e.employee_name, o.order_date, o.order_id, d.product_id, p.product_name
from employee e, order_header o, order_detail d, product p
where e.employee_id = o.employee_id and o.order_id = d.order_id and d.product_id = p.product_id
and e.employee_name = '다정한' and left(o.order_date, 4) = '2019';
select e.employee_id, e.employee_name, o.order_date, o.order_id, d.product_id, p.product_name
from employee e inner join order_header o
on e.employee_id = o.employee_id 
inner join order_detail d
on o.order_id = d.order_id 
inner join product p
on d.product_id = p.product_id
where e.employee_name = '다정한' and left(o.order_date, 4) = '2019';
...
where c.customer_id = h.customer_id and e.employee_id = h.employee_id and h.order_id = d.order_id and d.product_id = p.product_id
and p.product_name like '%청소기%';
select c.customer_id, e.employee_id, h.order_id, h.order_date, p.product_name
from order_header h inner join employee e 
...
select customer_name, customer_id, city, register_date, point from customer where city = (select city from customer where customer_name = '홍길동');
--      단, 고객 이름을 기준으로 오름차순 정렬해서 조회하세요.
select customer_name, customer_id, city, register_date, point from customer where city = (select city from customer where customer_name = '홍길동')
order by customer_name asc;

-- Q20) 하위 쿼리를 사용해 홍길동(gdhong) 고객보다 포인트가 많은 고객 이름, 아이디, 등록일, 포인트를 조회하고, 행번호를 추가하여 출력하세요.
select row_number() over() as no, customer_name, customer_id, register_date, point from customer where point > (select point from customer where customer_name = '홍길동');
