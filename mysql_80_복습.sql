...
select emp_id as 사원번호, emp_name, gender, hire_date, dept_id, phone, email from employee where dept_id = 'sys';

-- EMP_NAME '사원명' 별칭 수정
select emp_id, emp_name as 사원명, gender, hire_date,dept_id, phone, email as 사원명 from employee;


-- !! WHERE 조건절 컬럼으로 별칭을 사용할 수 있을까요???
-- 사원명이 홍길동인 사원을 별칭으로 조회??? ::: WHERE 조건절에서 별칭을 컬럼명으로 사용X 
select emp_id, emp_name as 사원명, gender, hire_date,dept_id, phone, email as 사원명 from employee where emp_name = '홍길동';
...
select emp_id, emp_name, email, phone, ifnull(salary, 0) from employee where dept_id ='stg';
...
select emp_id, emp_name, hire_date, ifnull(retire_date,curdate()) as retire_date from employee where dept_id = 'mkt';
/**************************************
...
select emp_id, emp_name, gender, hire_date, phone, email, salary as 급여 from employee where retire_date is not null order by 급여 desc;

-- 정보시스템(SYS) 부서 사원들 중 입사일이 빠른 순서, 급여를 많이 받는 순서로 정렬
-- HIRE_DATE, SALARY 컬럼은 '입사일','급여' 별칭으로 컬럼리스트 생성 후 정렬
select emp_id, emp_name, gender, hire_date as 입사일, phone, email, salary as 급여 from employee where dept_id = "sys" order by hire_date, 급여 desc;
...
-- 보너스가 1000 이상인 사원 조회
-- 보너스가 높은 사원 기준으로 정렬
select emp_id, emp_name, hire_date, email, salary, ifnull(salary*0.2, 50) as bonus from employee where salary*0.2>=1000 order by bonus desc;
...
select emp_id, emp_name, concat(emp_name,"/",ifnull(eng_name,'smith')), hire_date, phone, salary from employee;

-- (2) substring(문자열, 위치, 갯수) : 문자열 추출, 공백도 한문자 처리

-- 사원테이블의 사번, 사원명, 입사년도, 입사월, 입사일, 급여를 조회
select emp_id, emp_name, substring(hire_date,1,4) 년, substring(hire_date,6,2) 월, substring(hire_date,9,2) as 일, salary from employee;
...
-- 사번 기준으로 내림차순 정렬
select emp_id, emp_name, hire_date, phone, format(ifnull(salary,0),0) as salary, format(salary*0.2,0) as bonus 
from employee where left(hire_date,4) between '2016' and '2017' order by emp_id desc;
...
select concat(format(sum(salary),0),'만원') as 총액 from employee;

-- (2) avg(숫자) : 전체 평균을 구하는 함수
-- 사원들 전체의 평균을 조회, 3자리 구분, 앞에 '$' 표시
-- 소수점은 절삭
select concat('$',format(avg(salary),0)) from employee;

-- 정보시스템(sys) 부서 전체의 급여 총액과 평균을 조회
-- 3자리 구분, 마지막 '만원' 표시
select concat(format(sum(salary),0),'만원') as 총액, concat(format(avg(salary),0),'만원') as 평균 from employee where dept_id = 'sys';
...
-- 3자리 구분
select format(sum(salary),0), format(avg(salary),0), format(max(salary),0), format(min(salary),0) from employee; 
...
-- 급여 컬럼의 null은 제외
select dept_id, sum(salary), avg(salary) from employee group by dept_id having sum(salary)>=30000;
...
-- 부서별 사원수, 총급여, 평균급여 조회
select dept_id, sum(salary), avg(salary) from employee group by dept_id with rollup;
...
from employee e, department d, unit u
where e.dept_id = d.dept_id and d.unit_id = u.unit_id
and e.retire_date is null and left(e.hire_date,4) between '2014' and '2016'
...
on e.dept_id = d.dept_id
where e.retire_date is null and left(e.hire_date,4) between '2017' and '2018';
